...
class MyInput extends Component {
  constructor(props) {
    ...
    if (props.type == 'idcard') {
      console.warn("<MyInput /> 类型是身份证（idcard）时，无需传入 'regular' 属性校验。")
  ...
  componentDidUpdate() {
    ...
    let data = {}
    let { value, regular, type, disabled } = this.props
  ...
  onFocus(value) {
    let { capital, onFocus } = this.props
  ...
  render() {
    let { value, checkStatus } = this.state
    let { name, title, iconEvent, placeholder, maxLength, hideLine, disColor, titleWidth, isFocus, disabled } = this.props
    ...
    return (
      <div className={style.myInputBox} style={boxStyle}>
        {title ? (
