...
class CxyInputField extends Component {
  constructor(props) {
    super(props)
    // 作为首次有真实数据渲染的标识
    ...
    let initCheckStatus = ''
    // 创建正则表达式
    this.regularExperssion = props.regular ? props.regular : null

    if (this.initValue) {
      if (props.type == 'idcard') {
        if (props.regular) console.warn("<CxyInputField /> 类型是身份证（idcard）时，无需传入 'regular' 属性校验。")
  ...
  static propTypes = {
    ...
    left: PropTypes.string, // 容器 margin-left
    right: PropTypes.string // 容器 margin-right
  }
  ...
  componentDidUpdate() {
    ...
    if (value !== this.state.value) {
      this.regularExperssion = regular
      data.value = value
      // 禁用状态或 value 为空，checkStatus 设为 success。
      data.checkStatus = disabled || !value || this.regularExperssion.test(value) ? 'success' : 'error'
  ...
  render() {
    ...
    const { name, title, iconEvent, placeholder, maxLength, hideLine, disColor, titleWidth, disabled, left = '0.3rem', right = 0 } = this.props

    // 内联样式：字段校验失败、不可点击状态
    const inputStyle = disabled && disColor ? { color: disColor, WebkitTextFillColor: disColor, textFillColor: disColor } : {}
    // 无标题时，input设置左对齐
    ...
    return (
      <div className={style.cxyInputFieldBox} style={boxStyle}>
        {title ? (
          <div className={style.cxyInputFieldTitle}>
            <div>{title}</div>
            {iconEvent ? (
              <div
        ...
        <div className={style.cxyInputField}>
          <input
            name={name}
            id={name}
            value={value}
          ...
          />
          {!hideLine ? <div className={style.cxyInputFieldLine} /> : ''}
          {!hideLine ? (
            <div
              className={`${style.cxyInputFieldFocusLine} ${checkStatus == 'error' ? style.cxyInputFieldOnFocus : ''}`}
              style={checkStatus == 'error' ? { borderBottom: '1px solid #ff4444' } : {}}
            />
          ) : (
            ''
          )}
...
CxyInputField.defaultProps = {
  disabled: false,
  type: 'text',
  capital: false
}

export default CxyInputField
